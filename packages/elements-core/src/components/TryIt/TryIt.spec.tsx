...
import { HttpParamStyles, IHttpOperation } from '@swaggermen/types';
...
describe('TryIt', () => {
  ...
  it('Makes the correct basic request', async () => {
    ...
    expect(fetchMock.mock.calls[0][0]).toBe('https://todos.swaggermen.io/todos');
  ...
  it('uses cors proxy url, if provided', async () => {
    ...
    await waitFor(() => expect(fetchMock).toHaveBeenCalled());
    expect(fetchMock.mock.calls[0][0]).toBe('https://some.proxy.com/https://todos.swaggermen.io/todos');
  });
  ...
  it('replaces url variables with default values when making request', async () => {
    ...
    await waitFor(() => expect(fetchMock).toHaveBeenCalled());
    expect(fetchMock.mock.calls[0][0]).toBe('ftp://default-namespace.swaggermen.io/api/eu/todos');
  });
  ...
  describe('Mocking', () => {
    it('Shows mock button', () => {
      render(<TryItWithPersistence httpOperation={basicOperation} mockUrl="https://mock-todos.swaggermen.io" />);
    ...
    it('Invokes request with mocked data', async () => {
      ...
      expect(fetchMock.mock.calls).toEqual([
        [
          'https://mock-todos.swaggermen.io/todos',
        ...
        [
          'https://todos.swaggermen.io/todos',
          expect.objectContaining({
            method: 'GET',
            headers: {},
          }),
    ...
    it('Persists mocking options between operations', async () => {
      const { rerender } = render(
        <MosaicProvider>
          <PersistenceContextProvider>
            <TryIt httpOperation={putOperation} mockUrl="https://mock-todos.swaggermen.io" />
          </PersistenceContextProvider>
        </MosaicProvider>,
      );
      ...
      rerender(
        <MosaicProvider>
          <PersistenceContextProvider>
            <TryIt httpOperation={basicOperation} mockUrl="https://mock-todos.swaggermen.io" />
          </PersistenceContextProvider>
        </MosaicProvider>,
      );
  ...
  describe('Multiple Servers', () => {
    ...
    it('sends request to a chosen server url', async () => {
      ...
      expect(fetchMock.mock.calls[0][0]).toContain('https://todos-dev.swaggermen.io');
    });
    ...
    it('sends a request using server variable default values', async () => {
      ...
      expect(fetchMock.mock.calls[0][0]).toContain('http://x-1000.todos-pr.swaggermen.io');
    });
    ...
    it('sends a request using server variable modified values', async () => {
      ...
      expect(fetchMock.mock.calls[0][0]).toContain('https://x-123.todos-pr.swaggermen.io');
      fetchMock.mockClear();
      ...
      expect(fetchMock.mock.calls[0][0]).toContain('https://x-1000.todos-pr.swaggermen.io');
    });
