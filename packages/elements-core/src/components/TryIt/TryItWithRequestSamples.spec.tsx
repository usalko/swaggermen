...
describe('TryItWithRequestSamples', () => {
  ...
  it('reacts to mocking', async () => {
    render(<TryItWithRequestSamples httpOperation={putTodosOperation} mockUrl="https://mock-todos.swaggermen.io" />);
    ...
    const codeViewer = await screen.findByLabelText(/curl/);
    expect(codeViewer).toHaveTextContent(/https:\/\/mock-todos\.swaggermen\.io/);
  ...
  it('displays base url correctly', async () => {
    ...
    const codeViewer = await screen.findByLabelText(/curl/);
    expect(codeViewer).toHaveTextContent('ftp://default-namespace.swaggermen.io/api/eu/todos');
  });
  ...
  it('by default displays 1st server url in request samples', async () => {
    ...
    const codeViewer = await screen.findByLabelText(/curl/);
    expect(codeViewer).toHaveTextContent('https://todos.swaggermen.io');
  });
  ...
  it('changes request sample when changing server', async () => {
    ...
    const codeViewer = await screen.findByLabelText(/curl/);
    expect(codeViewer).toHaveTextContent('https://todos-dev.swaggermen.io');
  });
  ...
  it('changes request sample when changing server variables', async () => {
    ...
    const codeViewer = await screen.findByLabelText(/curl/);
    expect(codeViewer).toHaveTextContent('https://x-123.todos-pr.swaggermen.io');
  });
});
