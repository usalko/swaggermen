import { Provider as MosaicProvider } from '@swaggermen/mosaic';
...
describe('HttpService', () => {
  ...
  it('displays all server urls', () => {
    ...
    expect(serverUrl).toHaveTextContent('https://api.swaggermen.io');

    const secondServerUrl = screen.getByLabelText('Staging API');
    expect(secondServerUrl).toHaveTextContent('https://api.{environment}.swaggermen.io');
  ...
  it('generates names for servers without descriptions', () => {
    ...
    expect(secondServerUrl).toHaveTextContent('https://api.staging.swaggermen.io');
  ...
  it('keeps url variables in displayed url', () => {
    ...
    const serverUrl = screen.getByLabelText('Production API');
    expect(serverUrl).toHaveTextContent('{protocol}://{namespace}.swaggermen.io');
  });
  ...
  it('displays mock server url when embedded in Swaggermen Project', async () => {
    render(<ServerInfo servers={httpService.servers ?? []} mockUrl="https://foo.swaggermen.io/prism/123" />);

    const mockServer = screen.queryByLabelText('Mock Server');
    await waitFor(() => expect(mockServer).toHaveTextContent('https://foo.swaggermen.io/prism/123'));
  });
  ...
  describe('Additional information', () => {
    it('should render additional information', () => {
      const contact = {
        name: 'Developer',
        email: 'developer@swaggermen.io',
        url: 'https://swaggermen.io/contact-us/',
      };
      ...
      render(
        <AdditionalInfo id="a" contact={contact} license={license} termsOfService="https://swaggermen.io/terms/" />,
      );
...
describe('useSplitUrl hook', () => {
  it('should correctly distinguish variables from static parts', () => {
    let { result } = renderHook(() => useSplitUrl('https://{api}.swaggermen.io:{port}'));

    expect(result.current).toStrictEqual([
      { kind: 'static', value: 'https://' },
      { kind: 'variable', value: '{api}' },
      { kind: 'static', value: '.swaggermen.io:' },
      { kind: 'variable', value: '{port}' },
    ]);

    ({ result } = renderHook(() => useSplitUrl('{protocol}://swaggermen.io:{port}')));
    ...
    ({ result } = renderHook(() => useSplitUrl('https://{version}{username}.swaggermen.io')));
    ...
    ({ result } = renderHook(() => useSplitUrl('https://www.swaggermen.io')));

    expect(result.current).toStrictEqual([{ kind: 'static', value: 'https://www.swaggermen.io' }]);
  });
  ...
  it('should gracefully handle invalid input', () => {
    ...
    ({ result } = renderHook(() => useSplitUrl('{protocol://swaggermen.io:{api}')));

    expect(result.current).toStrictEqual([
      { kind: 'static', value: '{protocol://swaggermen.io:' },
      { kind: 'variable', value: '{api}' },
    ]);

    ({ result } = renderHook(() => useSplitUrl('https://{version}{username}.swaggermen.io{test')));

    expect(result.current).toStrictEqual([
      { kind: 'static', value: 'https://' },
      { kind: 'variable', value: '{version}' },
      { kind: 'variable', value: '{username}' },
      { kind: 'static', value: '.swaggermen.io{test' },
    ]);
  });
});
