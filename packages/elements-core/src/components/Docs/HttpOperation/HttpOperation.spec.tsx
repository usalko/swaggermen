...
import { SwaggermenOptionsProvider } from '../../../context/Options';
...
describe('HttpOperation', () => {
  describe('Header', () => {
    ...
    it('should correctly display with server variables at beginning, middle, and end', () => {
      ...
      expect(screen.queryAllByText(/{proto}:\/\/x-{pr}.todos-pr.swaggermen.io:{port}/)[0]).toBeInTheDocument();
      unmount();
    });
  });
  ...
  describe('Request Body', () => {
    ...
    it('request body selection in Docs should update TryIt', async () => {
      ...
      const requestSample = await screen.findByLabelText(
        'curl --request POST \\ --url https://todos.swaggermen.io/users \\ --header \'Content-Type: application/json\' \\ --data \'{ "name": "string", "age": 0 }\'',
      );
      ...
      const secondRequestSample = await screen.findByLabelText(
        "curl --request POST \\ --url https://todos.swaggermen.io/users \\ --header 'Content-Type: application/x-www-form-urlencoded' \\ --data name= \\ --data completed= \\ --data someEnum=a",
      );
  ...
  describe('Callbacks', () => {
    it('should display callback operation', async () => {
      ...
      expect(screen.queryAllByText(/https:\/\/todos-dev.swaggermen.io/).length).toEqual(1); // server url visible only in the main operation header, not in callback
  ...
  describe('Vendor Extensions', () => {
    it('should call rendorExtensionAddon', async () => {
      ...
      const { unmount } = render(
        <SwaggermenOptionsProvider renderExtensionAddon={vendorExtensionRenderer}>
          <HttpOperation
            data={httpOperation}
            layoutOptions={{
              hideTryItPanel: true,
    ...
    it('should display vendor extensions in body', async () => {
      const vendorExtensionRenderer = jest.fn().mockImplementation(props => {
        if ('x-swaggermen-info' in props.vendorExtensions) {
          return <div>Swaggermen Information Extension</div>;
        }
      ...
      expect(screen.queryByText('Swaggermen Information Extension')).toBeInTheDocument();
