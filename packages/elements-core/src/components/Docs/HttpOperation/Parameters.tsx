import { isPlainObject } from '@swaggermen/json';
import { JsonSchemaViewer } from '@swaggermen/json-schema-viewer';
import { HttpParamStyles, IHttpParam } from '@swaggermen/types';
...
const httpOperationParamsToSchema = ({ parameters, parameterType }: ParametersProps): JSONSchema7Object | null => {
  ...
  for (const p of sortedParams) {
    ...
    // TODO (CL): This can be removed when http operations are fixed https://github.com/swaggermenio/http-spec/issues/26
    ...
    if (isPlainObject(schema.properties)) {
      schema.properties![p.name] = {
        ...
        'x-swaggermen': {
          ...(isPlainObject(paramSchema['x-swaggermen']) ? paramSchema['x-swaggermen'] : {}),
          id: p.id,
        },
      };
    }
