import type { IMarkdownViewerProps } from '@swaggermen/markdown-viewer';
import { isArray } from '@swaggermen/mosaic';
import { IHttpOperation, IHttpService, IHttpWebhookOperation, INode } from '@swaggermen/types';
...
function isSwaggermenNode(maybeNode: unknown): maybeNode is INode {
...
export function isHttpService(maybeHttpService: unknown): maybeHttpService is IHttpService {
  return isSwaggermenNode(maybeHttpService) && 'name' in maybeHttpService && 'version' in maybeHttpService;
}

export function isHttpOperation(maybeHttpOperation: unknown): maybeHttpOperation is IHttpOperation {
  return isSwaggermenNode(maybeHttpOperation) && 'method' in maybeHttpOperation && 'path' in maybeHttpOperation;
}
...
export function isHttpWebhookOperation(
  ...
  return (
    isSwaggermenNode(maybeHttpWebhookOperation) &&
