...
import { Box, Icon, Input, useModalState } from '@swaggermen/mosaic';
...
const SearchWrapper = ({ projectIds, workspaceId }: SearchWrapperProps) => {
  ...
  const handleClick = (searchResult: NodeSearchResult) => {
    ...
    window.open(
      `https://${workspace?.workspace.slug}.swaggermen.io/docs/${searchResult.project_slug}${searchResult.uri}`,
      '_blank',
    );
