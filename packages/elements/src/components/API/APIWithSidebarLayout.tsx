...
import { ExtensionAddonRenderer } from '@swaggermen/swaggermen-core/components/Docs';
import { Flex, Heading } from '@swaggermen/mosaic';
...
export const APIWithSidebarLayout: React.FC<SidebarLayoutProps> = ({
  ...
  return (
    <SidebarLayout ref={container} sidebar={sidebar}>
      {node && (
        <SwaggermenOptionsProvider renderExtensionAddon={renderExtensionAddon}>
          <ParsedDocs
            key={relativePath}
            uri={relativePath}
            node={node}
...
export const Sidebar: React.FC<SidebarProps> = ({ serviceNode, logo, container, pathname, tree }) => {
  ...
  return (
    <>
      ...
      <PoweredByLink source={serviceNode.name} pathname={pathname} packageType="swaggermen" />
    </>
  );
};
