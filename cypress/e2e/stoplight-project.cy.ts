describe('Swaggermen component', () => {
  beforeEach(() => {
    cy.redirectUnpkgToLocalDistFolder('swaggermen');
  });

  describe('Api page', () => {
    it('loads api page correctly', () => {
      loadSwaggermenProjectPage();

      cy.intercept(`https://Swaggermen.io/api/v1/projects/cHJqOjYwNjYx/nodes/**`).as('getNode');
      cy.findByText('To-dos').click();
      cy.wait('@getNode');

      cy.findByText('https://todos.Swaggermen.io').should('exist');
      cy.findByText('https://Swaggermen.io/mocks/swaggermen-examples/studio-demo/389434').should('exist');
      ...
      /* With the change from displaying securitySchemes to security on the HttpService component,
          ...
          https://github.com/Swaggermenio/studio-demo - that may no longer exist. 
  ...
  describe('Operation page', () => {
    ...
    it('mocks response correctly', () => {
      ...
      cy.intercept('https://Swaggermen.io/mocks/**').as('getData');
      cy.findByRole('button', { name: /send api request/i }).click();
      cy.wait('@getData');
...
function loadSwaggermenProjectPage() {
  cy.intercept('https://Swaggermen.io/api/v1/projects/cHJqOjYwNjYx/nodes/**').as('getNode');
  cy.intercept(`https://Swaggermen.io/api/v1/projects/cHJqOjYwNjYx/table-of-contents`).as('getToc');
  cy.visit('/Swaggermen-project');
  cy.wait('@getNode');
  cy.wait('@getToc');
}
...
function visitNode(nodeId: string, nodeSlug: string) {
  cy.intercept(`https://Swaggermen.io/api/v1/projects/cHJqOjYwNjYx/nodes/${nodeId}-${nodeSlug}`).as('getNode');
  cy.intercept(`https://Swaggermen.io/api/v1/projects/cHJqOjYwNjYx/table-of-contents`).as('getToc');
  cy.visit(`/Swaggermen-project/${nodeId}-${nodeSlug}`);
  cy.wait('@getNode');
  cy.wait('@getToc');
}
